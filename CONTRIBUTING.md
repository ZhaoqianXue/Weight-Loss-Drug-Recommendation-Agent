...
2. Put reusable code in `src/weightloss/`; put active experimental configurations in `configs/` and keep historical experiments under `archive/`. Use `get_settings()` inside operations rather than assuming the working directory. Module import must not download models, read study CSVs or call services.
...
- `make check`: offline tests, existing dataset consistency, frontend assertions and local Markdown links. Historical migration audits are archived and do not constrain everyday directory changes.
...
Before a public release, maintainers must supply approved author/citation metadata and select a software license. Raw/derived review data, third-party terminology and reference PDFs have separate source permissions. No author list or license is inferred from local Git configuration. See [the roadmap](docs/roadmap.md) for the existing review-text release proposal.
