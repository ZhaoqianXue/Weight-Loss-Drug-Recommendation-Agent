...
def gpt_resolve_match(side_effect: str, ae_candidates: List[str]) -> Optional[str]:
    ...
    try:
        ...
        record_event('model_response', model=getattr(response, 'model', OPENAI_LLM_MODEL), request_id=getattr(response, '_request_id', None), response_id=getattr(response, 'id', None), side_effect=side_effect, candidates=ae_candidates, response=response.choices[0].message.content)
...
def standardize_side_effects(review_df: pd.DataFrame, ae_texts: List[str], 
    ...
    results = review_df.progress_apply(
        lambda row: process_row(row, get_model(), ae_texts, ae_embeddings), axis=1
    )
