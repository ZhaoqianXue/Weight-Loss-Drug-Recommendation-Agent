...
def get_extraction_chain():
    ...
    llm = ChatOpenAI(model="gpt-4.1-nano", temperature=0.2, api_key=os.environ.get("OPENAI_API_KEY", ""))
    ...
    return extraction_prompt | llm.with_structured_output(ExtractedData, include_raw=True)
...
def safe_extract_structured(text: str, drug_name: str, condition_from_csv: str = None, medication_duration: str = None) -> Optional[dict]:
    ...
    try:
        ...
        raw = result.get('raw')
        record_event('model_response', model='gpt-4.1-nano', response_id=getattr(raw, 'id', None), metadata=getattr(raw, 'response_metadata', {}), text=text, drug_name=drug_name, response=result['parsed'].model_dump() if result.get('parsed') else None, parsing_error=type(result['parsing_error']).__name__ if result.get('parsing_error') else None)
        if result.get('parsing_error') or result.get('parsed') is None:
            raise ValueError('Structured extraction response could not be parsed')
        extracted_dict = result['parsed'].model_dump()
...
if __name__ == "__main__":
    import pandas as pd
    INPUT_CSV_PATH = get_settings().raw
    OUTPUT_CSV_PATH = get_settings().path("results") / "extracted_reviews_top10.csv"
